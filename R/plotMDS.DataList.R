plotMDS.DataList <- function (x,top=500,labels=NULL,pch=NULL,cex=1,dim.plot=c(1,2),ndim=max(dim.plot),gene.selection="pairwise",xlab=NULL,ylab=NULL,method="logFC",prior.count=2,plot=TRUE,...) {
...
#	Check x
	...
	if (!all(is.finite(x$counts))) {
		stop("Missing or infinite counts not allowed")
	}

	nprobes <- nrow(x)
	nsamples <- ncol(x)
	if (nsamples < 3) {
		stop("Need at least 3 columns of data")
	}
...
#	Add random variate if necessary
	ndiff <- ndim-ncol(a1)
	if (ndiff > 0) {
		a1 <- cbind(a1, matrix(runif(ndiff*nsamples, -1e-6, 1e-6), ncol=ndiff, nrow=nsamples))
	}
