...
# Created by Mark Robinson. Last modified by Yunshun Chen, 19 March 2012.
	...
	if (!(class(object) %in% c("DataList", "DGELRT", "DGEExact"))) {
		stop("Currently only supports DataList/DGELRT/DGEExact objects as the object argument.")
	}
	if (is(object, "DataList") && is.null(object$samples$group)) {
