...
#	Convert factor to integer levels for efficiency
	...
	if (!is.null(design)) {
		if (ncol(design)!=ngroups) {
			stop("design matrix is not equivalent to a oneway layout")
		}
...
#		It is just a group indicator matrix?
		if (sum(designunique==1)==ngroups && sum(designunique==0)==(ngroups-1L)*ngroups) {
			design <- NULL
		}

#		If necessary, convert starting values to group fitted values
		if (!is.null(design) && !is.null(coef.start)) {
			coef.start <- coef.start %*% t(designunique)
		}
	}
