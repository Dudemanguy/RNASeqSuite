...
estimateDisp.DataList <- function(y, design=NULL, prior.df=NULL, trend.method="locfit", mixed.df=FALSE, tagwise=TRUE, span=NULL, min.row.sum=5, grid.length=21, grid.range=c(-10,10), robust=FALSE, winsor.tail.p=c(0.05,0.1), tol=1e-06, ...) {
#  Yunshun Chen.
...
#	Check covariate and trend
	...
	if (trend.method=="locfit" & mixed.df) {
		trend.method <- "locfit_mix"
	}
...
# last modified 29 June 2017    
	...
	if (!missing(i)) {
		...
		if (!identical(example, example[i,,drop=FALSE])) {
			isokay <- FALSE
		}
	}
	if (!missing(j)) {
		...
		if (!identical(example, example[,j,drop=FALSE])) {
			isokay <- FALSE
		}
	}
