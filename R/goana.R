...
#	Avoid argument collision with default method
	dots <- names(list(...))
	if ("universe" %in% dots) {
		stop("goana.DataLRT defines its own universe",call.=FALSE)
	}
	if ((!is.logical(trend) || trend) && "covariate" %in% dots) {
		stop("goana.DataLRT defines it own covariate",call.=FALSE)
	}
	ngenes <- nrow(de)
...
#	Check FDR
	if (!is.numeric(FDR) | length(FDR) != 1) {
		stop("FDR must be numeric and of length 1.")
	}
	if (FDR < 0 | FDR > 1) {
